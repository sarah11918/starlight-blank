...
First, [add an adapter integration](#add-an-adapters) for your server runtime to enable on-demand server rendering in your Astro project.
...
For a **highly dynamic app**, after adding an adapter, [set your build output configuration to `output: 'server'`](/en/reference/configuration-reference/#output) to **server-render all your pages by default**. This is the equivalent of opting out of prerendering on evey page.
