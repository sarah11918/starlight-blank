...
Renderers can be added through the Container API automatically for [static applications](/en/basics/rendering/#prerendered-pages), or cases where the container isn't called at runtime (e.g. testing with `vitest`).

For [on-demand rendered applications](/en/basics/rendering/#on-demand-rendered-routes), or cases where the container is called at runtime or inside other "shells" (e.g. PHP, Ruby, Java, etc.), renderers must be manually imported.
